...
class Indexer:
    ...
    def _index_file(self, filename: str, uri: str, metadata: Dict[str, Any], id: str = None) -> bool:
        ...
        # Some POC backends require corpus id as form part 'o' and expect it first
        try:
            with open(filename, 'rb') as file_handle:
                files, _, content_type = create_upload_files_dict(filename, metadata, self.parse_tables, self.cfg)
                files = {'o': (None, self.corpus_key), **files}
        ...
        if response.status_code == 201:
            logger.info(f"✓ Successfully indexed file {uri} to Vectara corpus {self.corpus_key}")
            if self.verbose:
                logger.debug(f"File {uri} indexed successfully")
            if self.store_docs:
            ...
            if self.reindex:
                ...
                if self.delete_doc(doc_id):
                    ...
                    try:
                        ...
                        if response.status_code == 201:
                            logger.info(f"✓ Successfully re-indexed file {uri} to Vectara corpus {self.corpus_key}")
                            if self.verbose:
                                logger.debug(f"File {uri} re-indexed successfully")
                            if self.store_docs:
                                store_file(filename, url_to_filename(uri), self.store_docs, self.store_docs_folder)
                            return True
                    except Exception as e:
    ...
    def index_document(self, document: Dict[str, Any], use_core_indexing: bool = False) -> bool:
        ...
        try:
            logger.debug(f"POSTing document {document['id']} to {api_endpoint}")
            response = self.session.post(api_endpoint, data=data, headers=post_headers)
            logger.debug(f"Response status: {response.status_code} for document {document['id']}")
        except Exception as e:
        ...
        # Handle the response
        if response.status_code == 201:
            logger.info(f"✓ Successfully indexed document {document['id']} to Vectara corpus {self.corpus_key}")
            if self.verbose:
                logger.debug(f"Document {document['id']} indexed successfully")
            if self.store_docs:
            ...
            if self.reindex:
                ...
                if self.delete_doc(document['id']):
                    ...
                    try:
                        response = self.session.post(api_endpoint, data=data, headers=post_headers)
                        if response.status_code == 201:
                            logger.info(f"✓ Successfully re-indexed document {document['id']} to Vectara corpus {self.corpus_key}")
                            if self.verbose:
                                logger.debug(f"Document {document['id']} re-indexed successfully")
                            if self.store_docs:
                                with open(f"{self.store_docs_folder}/{document['id']}.json", "w") as f:
                                    json.dump(document, f)
                            return True
    ...
    def index_segments(self, doc_id: str, texts: List[str], titles: Optional[List[str]] = None,
        ...
        logger.info(f"Uploading document {doc_id} to Vectara corpus {self.corpus_key}...")
        if self.verbose:
            logger.debug(f"Indexing document {doc_id} with json {str(document)[:500]}...")
